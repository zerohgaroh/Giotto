import { addWaiterOrder } from "@/lib/waiter-backend/backend";
import { noStoreJson, toErrorResponse } from "@/lib/waiter-backend/http";
import { requireWaiterSession } from "@/lib/waiter-backend/session";
import type { WaiterOrderInput } from "@/lib/waiter-backend/types";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type Params = {
  params: { tableId: string };
};

export async function POST(request: Request, { params }: Params) {
  try {
    const session = await requireWaiterSession();
    const body = (await request.json()) as { items?: WaiterOrderInput[] };
    const data = await addWaiterOrder({
      waiterId: session.waiterId,
      tableId: params.tableId,
      items: Array.isArray(body.items) ? body.items : [],
    });
    return noStoreJson(data);
  } catch (error) {
    return toErrorResponse(error);
  }
}
